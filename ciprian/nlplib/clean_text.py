...
class CleanText:
    ...
    def removeStopWords(self, text, language='EN', encoding='utf-8'):
        text = text.decode("utf8").encode(encoding)
        if language == 'EN':
            return ' '.join([word for word in text.split() if word not in cachedStopWords_en]).encode(encoding)
        elif language == 'FR':
            return ' '.join([word for word in text.split() if word not in cachedStopWords_fr]).encode(encoding)
    ...
    def cleanTextSimple(self, text, language = 'EN'):
        text = self.replaceUTF8Char(self.removeURLs(self.removeTags(self.removeScripts(text))))
        text = self.expandContractions(text, language)
        text = self.removeMultipleSpaces(text.strip())
        return text
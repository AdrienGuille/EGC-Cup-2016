...
stopwords_list = {
    'EN': [ item.strip() for item in open("stopwords.txt").readlines() ], 
    'FR': nltk.corpus.stopwords.words("french") + ["ce", "cet", "cette", "le", "les"]
    }

#recover_list = {"wa":"was", "ha":"has"}
#wl = nltk.WordNetLemmatizer()

def is_stopword(w, lang):
    return w in stopwords_list[lang]
#def lemmatize(w0):
...
class Vocabulary:
    def __init__(self, excluds_stopwords=True, wordfreq_threshold=10, lang='FR'):
        #if to word
    ...
    def gen_vocabs(self, corpus, prev_voca, prev_lda):
        ...
        if (prev_voca == None) and (prev_lda == None):
            for doc in corpus:
                for word in doc:
                    if (self.excluds_stopwords and is_stopword(word, self.lang)) or (len(word)<3):
                        pass
                    else:
...
#        return [conv(doc) for doc in corpus]
    ...
    def is_stopword_id(self, id):
        return self.vocas[id] in stopwords_list[self.lang]
